...
u8 CAN2_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
    ...
    GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化PD0,PD1
	  ...
	  //引脚复用映射配置
	  GPIO_PinAFConfig(GPIOD,GPIO_PinSource0,GPIO_AF_CAN1); //GPIOD0复用为CAN2
  	GPIO_PinAFConfig(GPIOD,GPIO_PinSource1,GPIO_AF_CAN1); //GPIOD1复用为CAN2
...
void CAN2_RX0_IRQHandler(void)                        //中断服务函数               
{
    if(CAN_GetITStatus(CAN2,CAN_IT_FMP0))
		{
      CAN_Receive(CAN2, CAN_FIFO0, &receive_data2);                 //接收实时数据
                                        //执行数据解析
		}
  	CAN_ClearITPendingBit(CAN2,CAN_IT_FMP0);               //清标志位
}
