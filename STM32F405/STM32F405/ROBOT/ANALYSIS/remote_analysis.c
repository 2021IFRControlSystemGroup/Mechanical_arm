...
//		RC_Ctl.mouse.y=-RC_Ctl.mouse.y;
	...
	Key_Analysis();
	//LostCountFeed(&Error_Check.count[1]);
}
...
//两种短按的触发方式 自行注释选择
//2017.4.29
u8 ButtonStatu_Verdict(KeyBoardTypeDef * Key)	//有两种检测方法，一种是以时间为分界点和后来者即为最终值的原理。另一种修复了长按状态前始终会存在另一状态的缺点
{																			//处理频率100HZ，还是放在定时器里，，
	if(Key->last==1)
	{
		Key->count++;
